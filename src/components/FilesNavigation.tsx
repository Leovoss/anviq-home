...
import { Activity, ArrowRight, BookOpen, ChevronLeft, ChevronRight, CircleHelp, Compass, FileText, Folder, History, House, Layers, LayoutGrid, List, Mail, PanelLeft, Server, ShieldCheck, UserRound, Workflow } from "lucide-react";
...
export function FilesOverview() {
  return <div className="files-overview files-screen">
    <div className="files-page-heading"><h1 className="sr-only">Anviq</h1><Logo /><p>Independent IT consulting and software</p></div>
...
export function FilesBrowse() {
  const groups = [
    ...
    { title: "Information", items: [{ label: "Engagement", href: "/explore/engagement", Icon: FileText }, { label: "Constraints", href: "/explore/constraints", Icon: ShieldCheck }, { label: "Questions", href: "/explore/questions", Icon: CircleHelp }, { label: "Activity", href: "/explore/activity", Icon: Activity }, { label: "Ship log", href: "/explore/ship-log", Icon: History }, { label: "About Anviq", href: "/explore/about", Icon: BookOpen }, { label: "Founder", href: "/explore/founder", Icon: UserRound }] },
  ];
...
export function FilesProjects({ view, setView, sort, setSort }: { view: FilesView; setView: (view: FilesView) => void; sort: FilesSort; setSort: (sort: FilesSort) => void }) {
  ...
  return <section className="files-projects files-screen" aria-labelledby="files-projects-title"><div className="files-page-heading"><h1 id="files-projects-title">Selected work</h1><p>Independent builds, end to end.</p></div>
    ...
    <p className="files-collection-note">Some are covered by discretion, built for internal use. Others are sold as products with their own landing page, where more detail is shared.</p>
  </section>;
}
